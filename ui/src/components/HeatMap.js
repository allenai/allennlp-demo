...
export default class HeatMap extends React.Component {
    ...
    componentDidUpdate(newProps) {
        if (
            ...
            this.props.normalization !== newProps.normalization
        ) {
    ...
    setOpacity(newProps) {
        ...
        this.setState(
            {
                opacity: opacity,
                minFilterOpacity: Math.min(...flatArray),
                maxFilterOpacity: Math.max(...flatArray),
            },
    ...
    setShowRowsAndColumns(newState) {
        if (newState.opacity && this.props.rowLabels && this.props.colLabels) {
            const showRowAt = this.props.rowLabels.map((label, index) => {
                return Math.max(...newState.opacity[index]) >= this.state.filterOpacity;
            });
            const transposeOpacity = newState.opacity[0].map((col, i) =>
                newState.opacity.map((row) => row[i])
            );
            const showColAt = this.props.colLabels.map((label, index) => {
    ...
    render() {
        const { data, colLabels, rowLabels, color = 'blue' } = this.props;

        const { activeRow, activeCol, showRowAt, showColAt, opacity } = this.state;
        ...
        return (
            <div className="heatmap-container">
                {this.props.includeSlider &&
                    this.state.minFilterOpacity !== this.state.maxFilterOpacity && (
                        <div className="slide_container">
                            <input
                                ...
                                className="slider"
                                onChange={(e) =>
                                    this.setState({ filterOpacity: Number(e.target.value) }, () =>
                                        this.setShowRowsAndColumns(this.state)
                                    )
                                }
                            />
                    ...
                    <div className="heatmap">
                        <div className="heatmap__ft">
                                    ...
                                    <table className="heatmap__col-labels">
                                        <tbody>
                                            <tr data-row="header">
                                                {colLabels.map(
                                                    (colLabel, colIndex) =>
                                                        (this.props.showAllCols ||
                                                            this.state.showColAt[colIndex]) && (
                                                            <th
                                                                className={`heatmap__label${
                                                                    colIndex === activeCol
                            ...
                            <div className="heatmap__tr">
                                ...
                                {/* END .heatmap__td */}
                                <div
                                    className={`heatmap__td heatmap__datagrid-container heatmap__datagrid-container--${color}`}>
                                    {/* BEGIN Data Grid */}
                                    <table>
                                        <tbody>
                                            {rowLabels.map(
                                                    ...
                                                    this.state.showRowAt[rowIndex] && (
                                                        <tr
                                                            ...
                                                            {colLabels.map(
                                                                (colLabel, colIndex) =>
                                                                    (this.props.showAllCols ||
                                                                        this.state.showColAt[
                                                                            colIndex
                                                                        ]) && (
                                                                        <td
                                                                            key={`${colLabel}_${colIndex}_${rowLabel}_${rowIndex}`}
                                                                            className="heatmap__cell"
                                                                            style={{
                                                                                backgroundColor: `rgba(${supportedColors[
                                                                                    color
                                                                                ].join(',')},${
                                                                                    opacity[
                                                                                        rowIndex
                ...
                {/* BEGIN Tooltip */}
                {
                    activeRow !== null && activeCol !== null ? (
                        <ReactTooltip
                            ...
                            <span className="heatmap-tooltip__meta">
                                <strong>Row:</strong> {rowLabels[activeRow]}
                            </span>
                            <span className="heatmap-tooltip__meta">
                                <strong>Column:</strong> {colLabels[activeCol]}
                            </span>
                        </ReactTooltip>
                    ) : null /* END Tooltip */
                }
