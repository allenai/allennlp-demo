...
class DemoInput extends React.Component {
    ...
    render() {
            ...
            switch (field.type) {
                ...
                case 'SELECT':
                    input = (
                        ...
                        <FormSelect
                            value={this.state[field.name] || field.options[0]}
        ...
        return (
            <React.Fragment>
                <FormInstructions>
                    <span>{exampleLabel || 'Enter text or'}</span>
                    <Select
                        dropdownMatchSelectWidth={false}
                        disabled={outputState === 'working'}
                ...
                <RunButtonArea>
                    <Button
                        ...
                        onClick={() => this.props.runModel(this.cleanInputs())}>
                        Run
                        <RightOutlined />
                    </Button>
                </RunButtonArea>
...
function RenderOptions(examples, groupIndex, fields) {
    return examples.map((example, exampleIndex) => {
        ...
        return (
            <Select.Option value={encodedName} key={encodedName}>
                {makeSnippet(example, fields)}
            </Select.Option>
        );
    });
}
