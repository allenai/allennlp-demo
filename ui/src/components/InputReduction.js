    ...
    while (idx2 <= reducedInput.length) {
        ...
        if (originalInput[idx] === reducedInput[idx2]) {
            ...
            while (idx <= originalInput.length && originalInput[idx] !== reducedInput[idx2]) {
                ...
                reducedStringColorized.push(
                    <BlankToken key={idx}>{originalInput[idx]}</BlankToken>
                );
                idx++;
            }
        }
    ...
    render() {
        ...
        if (reducedInput === undefined) {
            if (this.state.loading) {
                displayText = (
                    <div>
                        <p style={{ color: '#7c7c7c' }}>Loading reduction...</p>
                    </div>
                );
            } else {
                displayText = (
                    <div>
                        <p style={{ color: '#7c7c7c' }}>
                            Press "reduce input" to run input reduction.
                        </p>
                        {runButton}
                    </div>
                );
        ...
        return (
            <>
                <p>
                    ...
                    removes as many words from the input as possible without changing the model's
                    prediction.
                </p>
                {displayText}
            </>
