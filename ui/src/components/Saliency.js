...
import { GRAD_INTERPRETER, IG_INTERPRETER, SG_INTERPRETER } from './InterpretConstants';
                ...
                <Collapse>
                    <Collapse.Panel header="Simple Gradients Visualization">
                        <SaliencyComponent
                            ...
                            interpretModel={interpretModel(requestData, GRAD_INTERPRETER)}
                    ...
                    <Collapse.Panel header="Integrated Gradients Visualization">
                        <SaliencyComponent
                            ...
                            inputTokens={inputTokens}
                            inputHeaders={inputHeaders}
                            interpretModel={interpretModel(requestData, IG_INTERPRETER)}
                            interpreter={IG_INTERPRETER}
                        />
                    </Collapse.Panel>
                    <Collapse.Panel header="SmoothGrad Visualization">
                        <SaliencyComponent
                            ...
                            inputTokens={inputTokens}
                            inputHeaders={inputHeaders}
                            interpretModel={interpretModel(requestData, SG_INTERPRETER)}
                            interpreter={SG_INTERPRETER}
                        />
                    </Collapse.Panel>
                </Collapse>
            </OutputField>
    ...
    colorize(tokensWithWeights, topKIdx) {
        ...
        tokensWithWeights.forEach((obj, idx) => {
            colorizedString.push(
                ...
                <label
                    key={idx}
                    data-tip={(1 - obj.weight).toFixed(3)}
    ...
    render() {
        ...
        if (interpretData === undefined) {
            if (this.state.loading) {
                displayText = (
                    <div>
                        <p style={{ color: '#7c7c7c' }}>Loading interpretation...</p>
                    </div>
                );
            } else {
                displayText = (
                    <div>
                        <p style={{ color: '#7c7c7c' }}>
                            Press "interpret prediction" to show the interpretation.
                        </p>
                        {runButton}
                    </div>
                );
            ...
            for (let i = 0; i < inputTokens.length; i++) {
                ...
                const saliencyMap = (
                    <div key={i}>
                        ...
                        />
                        <br />
                        <span style={{ color: '#72BCFF' }}>
                            Visualizing the top {k} most important words.
                        </span>
                        <br />
                        <br />
                    </div>
