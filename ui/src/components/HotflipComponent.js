...
export default class HotflipComponent extends React.Component {
    ...
    render() {
        ...
        // data is available, display the results of Hotflip
            ...
            [originalString, flippedString] = colorizeTokensForHotflipUI(
                hotflipData.original,
                hotflipData.final[0]
            );
        ...
        const target =
            targeted === undefined ? null : (
                <p>
                    {' '}
                    Leave blank to allow any change (untargeted). For targeted attacks, enter a
                    single token that you want to flip the mask to:{' '}
                    <FormInput type="text" onChange={this.updateTargetWord} />{' '}
                </p>
            );

        const buttonDisplay =
            flippedString !== ' ' && targeted === undefined ? null : (
                <div>
        ...
        const controlDisplay =
            this.state.loading && flippedString === ' ' ? (
                <div>
                    <p style={{ color: '#7c7c7c' }}>Loading attack...</p>
                </div>
            ) : (
                buttonDisplay
            );
        ...
        const resultDisplay =
            flippedString === ' ' ? (
                ...
                <div>
                    {context !== ' ' ? context : ''}
                    <p>
                        <strong>Original Input:</strong> {originalString}
                    </p>
                    <p>
                        <strong>Flipped Input:</strong> {flippedString}
                    </p>
                    <p>
                        <b>Prediction changed to:</b> {newPrediction}
                    </p>
                </div>
            );
        ...
        return (
            <>
                <p>
                    ...
                    flips words in the input to change the model's prediction. We iteratively flip
                    the input word with the highest gradient until the prediction changes.
                </p>
                {resultDisplay}
                {controlDisplay}
            </>
