...
const description = (
    <span>
        <span>
            Natural language to SQL interfaces allow users to query data in relational databases
            without writing SQL queries. This demo is an implementation of an encoder-decoder
...
const ActionInfo = ({ action, tokenized_utterance }) => {
    ...
    const utterance_attention_heatmap =
        utterance_attention.length > 0 ? (
            <div className="heatmap, heatmap-tile">
                <HeatMap
                    colLabels={['Prob']}
                    rowLabels={tokenized_utterance}
                    data={utterance_attention}
                />
            </div>
        ) : (
            ''
