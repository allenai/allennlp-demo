...
function addToUrl(output, choice) {
    if ('history' in window) {
        window.history.pushState(
            null,
            null,
            '?text=' + encodeURIComponent(output + (choice || ''))
        );
    }
}
...
const description = (
    <span>
        <p>
            Language modeling is the task of determining the probability of a given sequence of
            words occurring in a sentence.{' '}
        </p>
        <p>
            This demonstration uses the public 345M parameter{' '}
            <a href="https://github.com/openai/gpt-2" target="_blank" rel="noopener noreferrer">
                OpenAI GPT-2
            </a>{' '}
            language model to generate sentences.
            <br />
            <br />
            Provide some initial text, and the model will generate a list of candidates for the next
            few words using{' '}
            <a
                href="https://api.semanticscholar.org/CorpusID:76662039"
                target="_blank"
                rel="noopener noreferrer">
                Stochastic Beam Search
            </a>
            . You can click on one of those candidate to choose it and continue, or you can keep
            typing. Click the left arrow at the bottom to undo your last choice.
        </p>
    </span>
);
...
const probabilitiesNote = (
    <span>
        <p>
            {' '}
            Note: The prediction percentages are normalized across these five sequences. The true
...
const Attacks = ({ attackData, attackModel, requestData }) => {
    ...
    if (attackData && attackData.hotflip) {
        ...
        hotflipData.new_prediction = cleanTokensForDisplay([
            hotflipData.outputs.top_tokens[0][0],
        ])[0];
...
class App extends React.Component {
    ...
    setOutput(evt) {
        ...
        if (value) {
            // TODO(michaels): I shouldn't need to do this
            ...
            // Update text input without request to backend server
            this.setState({
                output: value,
                top_tokens: null,
                logits: null,
    ...
    render() {
        const requestData = { sentence: this.state.output };
        const interpretData = this.state.interpretData;
        const attackData = this.state.attackData;
        let tokens = [];
            ...
            <Wrapper classname="model">
                ...
                <div hidden className="model__content">
                    <MySaliencyMaps
                        ...
                        tokens={tokens}
                        interpretModel={this.interpretModel}
                        requestData={requestData}
                    />
                    <Attacks
                        attackData={attackData}
                        attackModel={this.attackModel}
                        requestData={requestData}
                    />
                </div>
            </Wrapper>
...
const Choices = ({ index, top_tokens, choose, probabilities }) => {
    if (!top_tokens) {
        return null;
    }
        ...
        return (
            <ListItem key={`${idx}-${cleanWord}`}>
                <ChoiceItem onClick={() => choose(cleanWord)}>
                    <Probability>{prob}</Probability> <Token>{displaySeq}</Token>
                </ChoiceItem>
            </ListItem>
        );
    });
        ...
        <ListItem key="go-back">
            {'history' in window ? (
                <UndoButton onClick={goBack}>
                    <Probability>←</Probability> <Token>Undo</Token>
                </UndoButton>
            ) : null}
        </ListItem>
    );
