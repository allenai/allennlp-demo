...
const description = (
    <span>
        <span>
            ...
            <a href="https://nlp.stanford.edu/software/sempre/wikitable/">
                {' '}
                WikiTableQuestions
            </a>{' '}
            dataset. The model is a re-implementation of the parser in the
...
const Output = ({ responseData }) => {
    ...
    return (
        <div className="model__content answer">
            <OutputField label="Answer">{answer}</OutputField>

            <OutputField label="Logical Form">
                <SyntaxHighlight language="lisp">{logical_form[0].toString()}</SyntaxHighlight>
            </OutputField>
                ...
                <Collapse defaultActiveKey={['default']}>
                    ...
                    <Collapse.Panel header="Entity linking scores">
                        <HeatMap
                            ...
                            rowLabels={entities}
                            data={linking_scores}
                            normalization="log-per-row-with-zero"
                        />
                    </Collapse.Panel>
                    {feature_scores && (
