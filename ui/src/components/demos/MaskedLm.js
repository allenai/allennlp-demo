...
const description = (
    <span>
        <p>
            Masked language modeling is a fill-in-the-blank task, where a model uses the context
        ...
        <p>
            ...
            , the first large transformer to be trained on this task. Enter text with one or more
            "[MASK]" tokens and the model will generate the most likely substitution for each. Note
            that in the default example, BERT demonstrates gender bias in that it thinks the doctor
            is more likely a man ("his") than a woman ("her"). An important issue in NLP is how to
            understand and address such biases in our linguistic models.
        </p>
    </span>
);
...
const Choices = ({ index, words, probabilities }) => {
    if (!words) {
        return null;
    }
    ...
    const lis = words[index].map((word, idx) => {
        ...
        return (
            <ListItem key={`${idx}-${cleanWord}`}>
                <ChoiceItem>
                    <Probability>{prob}</Probability> <Token>{cleanWord}</Token>
                </ChoiceItem>
            </ListItem>
        );
    });

    return <ChoiceList>{lis}</ChoiceList>;
};
