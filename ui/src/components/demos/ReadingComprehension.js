...
const description = (
    <span>
        Reading comprehension is the task of answering questions about a passage of text to show
        that the system understands the passage.
    </span>
);
...
const taskModels = [
    ...
    },
    {
        ...
        desc: (
            <span>
                Reimplementation of{' '}
                <a href="https://arxiv.org/abs/1611.01603">BiDAF (Seo et al, 2017)</a>, or
                Bi-Directional Attention Flow, a widely used reading comprehension baseline that
                achieved state-of-the-art accuracies on the SQuAD dataset (Wikipedia sentences) in
                early 2017.
            </span>
        ),
        modelId: 'bidaf',
    ...
    },
    {
        ...
        desc: (
            <span>
                A reading comprehension model patterned after the proposed model in{' '}
                <a href="https://arxiv.org/abs/1810.04805">Devlin et al</a>, with improvements
    ...
    },
    {
        ...
        desc: (
            <span>
                An augmented version of QANet that adds rudimentary numerical reasoning ability,
                trained on{' '}
                <a href="https://arxiv.org/pdf/1903.00161.pdf">DROP (Dua et al., 2019)</a>, as
            ...
            <OutputField label="Model Internals">
                <Collapse>
                    <Collapse.Panel header="Passage to Question attention">
                        <p>
                            For every passage word, the model computes an attention over the
                            question words. This heatmap shows that attention, which is normalized
                            for every row in the matrix.
                        </p>
                        <HeatMap
                            colLabels={question_tokens}
                            rowLabels={passage_tokens}
...
const NoAnswer = () => {
    return <OutputField label="Answer">No answer returned.</OutputField>;
};
    ...
    if (original && highlightSpans && highlightStyles) {
        ...
        highlightSpans.forEach((s, sIndex) => {
            if (s[0] > curIndex) {
                ...
                spanList.push(
                    <span key={`${curIndex}_${s[0]}`}>{original.slice(curIndex, s[0])}</span>
                );
                curIndex = s[0];
            }
            ...
            if (s[1] > curIndex) {
                spanList.push(
                    <span key={`${curIndex}_${s[1]}`} className={highlightStyles[sIndex]}>
                        {original.slice(curIndex, s[1])}
                    </span>
                );
                curIndex = s[1];
            }
        ...
        if (curIndex < original.length) {
            spanList.push(
                <span key={`${curIndex}_${original.length}`}>{original.slice(curIndex)}</span>
            );
        }
        return <span>{spanList.map((s) => s)}</span>;
    }
    return null;
};
    ...
    if (interpretData) {
        simpleGradData =
            GRAD_INTERPRETER in interpretData
                ? getGradData(interpretData[GRAD_INTERPRETER].instance_1)
                : undefined;
        integratedGradData =
            IG_INTERPRETER in interpretData
                ? getGradData(interpretData[IG_INTERPRETER].instance_1)
                : undefined;
        smoothGradData =
            SG_INTERPRETER in interpretData
                ? getGradData(interpretData[SG_INTERPRETER].instance_1)
                : undefined;
    }
...
const Attacks = ({ attackData, attackModel, requestData }) => {
    ...
    const inputReduction =
        model && model.toLowerCase().includes('naqanet') ? null : (
            <InputReductionPanel>
        ...
        switch (answerType) {
            case 'passage_span': {
                ...
                if (question && passage && spans && value) {
                    return (
                        <section>
                            <OutputField label="Answer">{value}</OutputField>
                            ...
                            <OutputField label="Passage">
                                <MultiSpanHighlight
                                    ...
                                    highlightStyles={spans.map((_) => 'highlight__answer')}
                                />
                            </OutputField>

                            <OutputField label="Question">{question}</OutputField>
                ...
                if (question && passage && spans && value) {
                    return (
                        <section>
                            ...
                            <OutputField label="Passage">{passage}</OutputField>
            ...
            case 'count': {
                ...
                if (question && passage && count !== undefined) {
                    return (
                        <section>
                            <OutputField label="Answer">{count}</OutputField>
                    ...
                    return (
                        <section>
                            ...
                            <OutputField label="Explanation">
                                ...
                                <ArithmeticEquation numbers={numbers} /> = {value}.
                            </OutputField>
            ...
            default: {
                // old best_span_str path used by BiDAF model
                ...
                if (question && passage && bestSpanStr) {
                    ...
                    return (
                        <section>
                            <OutputField label="Answer">{bestSpanStr}</OutputField>
        ...
        [
            {
                passage:
                    'In 1085, Guadalajara was retaken by the Christian forces of Alfonso VI . The chronicles say that the Christian army was led by Alvar Fanez de Minaya, one of the lieutenants of El Cid. From 1085 until the Battle of Las Navas de Tolosa in 1212, the city suffered wars against the Almoravid and the Almohad Empires. In spite of the wars, the Christian population could definitely settle down in the area thanks to the repopulation with people from the North  who received their first fuero in 1133 from Alfonso VII.In 1219, the king Fernando III gave a new fuero to the city .During the reign of Alfonso X of Castile, the protection of the king allowed the city to develop its economy by protecting merchants and allowing markets.',
                question:
                    'How many years did the city suffer wars against Almoravid and the Almohad Empires?',
            },
            {
                passage:
                ...
                question:
                    'In May 2016, how many members of the Armed Forces were not active military personnel?',
            },
            {
                passage:
                ...
                question:
                    'How many more Korean immigrant men were there compared to women and children?',
            },
            {
                passage:
                ...
                question:
                    'How many yards was the difference between the longest and shortest TD runs?',
            },
            {
                passage:
