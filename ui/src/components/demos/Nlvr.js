...
const description = (
    <span>
        <span>
            Semantic parsing maps natural language to machine language. This page demonstrates a
            semantic parsing model on the
            <a href="http://lic.nlp.cornell.edu/nlvr/"> Cornell NLVR</a> dataset. The model is the
            ...
            <a href="https://www.semanticscholar.org/paper/Iterative-Search-for-Weakly-Supervised-Semantic-Dasigi-Gardner/af17ccbdae4cbd1b67d6ab359615c8000f8fb66f">
                {' '}
                NAACL 2019 paper by Dasigi, Gardner, Murty, Zettlemoyer, and Hovy
            </a>
            . The structured representation is not particularly useful here, unless you know what
            you're doing. We recommend just seeing how the sentence maps to a logical form.
        </span>
    </span>
);
...
const fields = [
    ...
    {
        name: 'structured_rep',
        label: 'Structured (JSON) Representation',
        type: 'TEXT_AREA',
...
const ActionInfo = ({ action, sentence_tokens }) => {
    ...
    const question_attention_heatmap =
        question_attention.length > 0 ? (
            <div className="heatmap, heatmap-tile">
                <HeatMap
                    colLabels={['Prob']}
                    rowLabels={sentence_tokens}
                    data={question_attention}
                />
            </div>
        ) : (
            ''
...
const Output = ({ responseData }) => {
    ...
    return (
        <div className="model__content answer">
            <OutputField label="Answer">{denotations[0]}</OutputField>

            <OutputField label="Logical Form" suppressSummary>
                <SyntaxHighlight language="lisp">{logical_form}</SyntaxHighlight>
            </OutputField>
            ...
            <OutputField label="Model internals">
                <Collapse defaultActiveKey={['default']}>
                    <Collapse.Panel header="Predicted production rules" key="default">
                        <PanelDesc>
                            The sequence of grammar production rules predicted by the model, which
                            together determine an abstract syntax tree for the program shown above.
                        </PanelDesc>
